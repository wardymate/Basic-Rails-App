require 'rails_helper'
 ...
 # #8
       ...
       ##THIS TEST STARTED FAILING WHEN I REFACTORED MY TESTS USING FACTORY GIRL IN CHECKPOINT 45: PUBLIC PROFILES
     ...
     describe 'DELETE destroy' do
      ...
      #    favorite = my_user.favorites.where(post: my_post).create
      #    expect( my_user.favorites.find_by_post_id(my_post.id) ).not_to be_ni
      #    delete :destroy, { post_id: my_post.id, id: favorite.id }
      #    expect( my_user.favorites.find_by_post_id(my_post.id) ).to be_nil
      #  end
     end
